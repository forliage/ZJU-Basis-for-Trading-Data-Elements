...
*1.*根据信息设计中的显示原理，我们总可以不失一般性地将信号机制简化为直接向接收方推荐一个行动。发送方设计的机制使得接收方总是愿意遵循其推荐。

在这个问题中，法官的行动是“判决有罪”或“无罪释放”。因此，我们可以将检察官的信号集简化为两个直接的“建议”：
- 信号$s_c$：“建议判决有罪”
- 信号$s_a$：“建议无罪释放”

一个信号机制因此可以被一个条件概率矩阵完全定义，该矩阵描述了在不同真实状态下，发送每个建议信号的概率。我们设：
- $p = P(s_c | omega = G)$：当被告有罪时，检察官提供“建议有罪”的证据的概率。
- $q = P(s_c | omega = I)$：当被告无罪时，检察官提供“建议有罪”的证据的概率。

那么，提供“建议无罪”的证据的概率就是：
- $P(s_a | omega = G) = 1 - p$
- $P(s_a | omega = I) = 1 - q$

因此，需要考虑的信号机制的形式可以由一个二维向量$(p,q)$来刻画，其中$p,q in [0,1]$。检察官的目标就是选择最优的$(p,q)$。 

*2.* 一个完全诚实或完全揭示的信号机制会完美地揭示被告的真实类型。
- 如果被告有罪，检察官 100% 提供定罪证据。
- 如果被告无罪，检察官 100% 提供脱罪证据。

在这种机制下：
- 当被告有罪时$(omega = G)$，法官收到的后验信念$mu = P(G | s_G) = 1$。 
- 当被告无罪时$(omega = I)$，法官收到的后验信念$mu = P(G | s_I) = 0$。

法官的决策与效用:
- 当后验信念$mu=1$时，因为$1 >= 0.5$，法官会判决有罪。这是正确的判决。
- 当后验信念$mu=0$时，因为$0 < 0.5$，法官会无罪释放。这也是正确的判决。
- 由于法官在任何情况下都做出了正确的判决，其效用始终为 1。因此，法官的期望效用为 1。

检察官的效用:
- 检察官只有在被告被判决有罪时才获得效用 1。
- 在完全诚实的机制下，只有当被告真实为有罪时，他才会被判有罪。
- 这种情况发生的概率等于被告有罪的先验概率，即$P("有罪")= mu_0 =0.3$。
- 因此，检察官的期望效用为$0.3 times 1 + 0.7 times 0 = 0.3$。 

*3.* 要找到最优机制，我们可以使用凹化的方法。检察官的期望效用可以被看作是法官后验信念$mu$的函数。

定义检察官的价值函数$v(mu)$，该函数表示当法官的后验信念为$mu$时，检察官能获得的效用。
$
v(mu) = cases(
  1 "if " mu >= 0.5 ("法官判有罪"),
  0 "if " mu < 0.5("法官无罪释放"),
...
$
这是一个在$mu=0.5$处跳跃的阶梯函数。

寻找价值函数的凹化包络$hat(v) (mu)$，检察官能获得的最大期望效用，等于其价值函数$v(mu)$的最小凹包络在先验信念$mu_0 = 0.3$处的值。对于所有$mu < 0.5$，这个凹包络是一条连接点$(0,v(0))=(0,0)$和点$(0.5,v(0.5))=(0.5,1)$的直线。该直线的方程为$ y = (1-0)/(0.5-0) x = 2 x $

计算检察官的最优效用:在先验$mu_0 = 0.3$处，检察官的最优期望效用为：
$ bb(E) [U_P]^ast = hat(v) (0.3) = 2 times 0.3 = 0.6 $
所以，检察官在最优信号机制下的效用为 0.6。

推导后验概率的分布，最优机制会将先验信念$mu_0$分裂成支持凹包络的几个后验信念点。在本例中，先验$mu_0 = 0.3$位于连接点$mu_L = 0$和$mu_H = 0.5$的线段上。因此，最优机制只会产生两个后验信念:$mu=0$和$mu=0.5$。根据信念鞅性，期望的后验信念必须等于先验信念：
$ bb(E) [mu] = mu_0 $
设$P(mu = 0.5)$为机制产生 0.5 后验信念的概率，则$P(mu=0)=1 - P(mu=0.5)$。 
$ P(mu = 0.5) dot 0.5 + P(mu = 0) dot 0 = 0.3 $
$ P(mu = 0.5) dot 0.5 = 0.3 ==> P(mu = 0.5) = (0.3)/(0.5) = 0.6 $
因此，$P(mu=0) = 1 - 0.6 = 0.4$。 

所以，最优信号机制下法官后验概率的分布为：以 0.6 的概率形成后验信念$mu=0.5$，以 0.4 的概率形成后验信念$mu = 0$。 

*4.* 现在我们来求解产生上述后验分布的具体信号机制$(p,q)$。 
- 信号$s_c$导致后验信念$mu = 0.5$。 
- 信号$s_a$导致后验信念$mu = 0$。

利用后验信念$mu (s_c) = 0.5$，根据贝叶斯法则：
$ mu(s_c) = P(G | s_c) = (P(s_c | G) P(G))/(P(s_c | G) P(G) + P(s_c | I) P(I)) = 0.5 $
代入$P(s_c | G) = p, P(s_c | I) = q, P(G) = 0.3, P(I) = 0.7$:
$ (p dot 0.3)/( p dot 0.3 + q dot 0.7) = 0.5 $
$ ==> 3 p = 7 q $
利用后验信念$mu(s_a) = 0$:
$ mu(s_a) = P(G| s_a) = (P(s_a | G) P(G))/(P(s_a | G) P(G) + P(s_a | I) P(I)) = 0 $
代入$P(s_a | G) = 1 - p, P(s_a | I) = 1 - q$： 
$ ((1 - p) dot 0.3)/((1 - p) dot 0.3 + (1 - q) dot 0.7) = 0 $
要使该分数为 0，分子必须为 0。因此：
$ (1 - p) dot 0.3 = 0 ==> p = 1 $
解得： 
$ q = 3/7 $
检察官的最优信号机制为:该机制由概率$(p,q) = (1, 3/7)$定义，其含义是：
- 当被告确实有罪时，检察官总是提供“建议有罪”的证据$(p = P(s_c | G) = 1)$。 
- 当被告是无罪的时，检察官以$3/7$的概率提供“建议有罪”的证据，以$1 - 3/7 = 4/7$的概率提供“建议无罪”的证据$(q = P(s_c | I) = 3/7)$。 

这个机制通过在被告无罪时进行策略性的信息模糊，成功地将法官的先验信念（0.3，不足以定罪）分裂成一个可以定罪的后验信念（0.5）和一个不能定罪的后验信念（0），从而最大化了检察官的期望效用。
...
在没有额外信息的情况下，买家只能根据其先验信念$mu_0 (omega_1) = 0.7$和$mu_0 (omega_2) = 0.3$来选择一个能最大化其期望效用的行动。

计算选择行动$a_1$的期望效用:
$ bb(E) [u(a_1)] = mu_0 (omega_1) dot u(a_1, omega_1) + mu_0 (omega_2) dot u(a_1, omega_2) $
$ bb(E) [u(a_1)] = (0.8 times 2) + (0.3 times 0) = 1.4 $
计算选择行动$a_2$的期望效用:
$ bb(E) [u(a_2)] = mu_0 (omega_1) dot u(a_2, omega_1) + mu_0 (omega_2) dot u(a_2, omega_2) $
$ bb(E) [u(a_2)] = (0.7 times 0) + (0.3 times 3) = 0.9 $
因为$bb(E) [u(a_1)] = 1.4 > bb(E) [u(a_2)] = 0.9$，一个理性的买家在没有其他信息时会固定选择行动$a_1$。 

因此，无信息时买家的最大期望效用为$bb(E) [U("无信息")] = 1.4$。 

当买家可以使用信号机制时，他会先观察信号，然后根据信号更新自己的信念（形成后验信念），再基于后验信念选择最优行动。

计算每个信号出现的边缘概率:信号$s_1$出现的概率为：
$ P(s_1) = P(s_1 | omega_1) P(omega_1) + P(s_1 | omega_2) P(omega_2) $
$ P(s_1) = (0.9 times 0.7) + (0.7 times 0.3) = 0.84 $
信号$s_2$出现的概率为：
$ P(s_2) = P(s_2 | omega_1) P(omega_1) + P(s_2 | omega_2) P(omega_2) $
$ P(s_2) = (0.1 times 0.7) + (0.3 times 0.3) = 0.16 $

计算每种信号下的后验信念:我们使用贝叶斯法则来更新信念。

观察到信号$s_1$后: 后验信念$ mu(omega_1 | s_1) = P(omega_1 | s_1) = (P(s_1 | omega_1) P(omega_1))/(P(s_1)) = 0.63/0.84 = 0.75 $ 因此，$ mu(omega_2 | s_1) = 1 - 0.75 = 0.25 $

观察到信号$s_2$后:后验信念$ mu(omega_1 | s_2) = P(omega_1 | s_2) = (P(s_2 | omega_1) P(omega_1))/(P(s_2)) = 7/(16) $ 因此，$ mu(omega_2 | s_2) = 1 - (7)/(16) = 9/(16) $

计算每种信号下的最优行动和期望效用:

当观察到$s_1$时 (后验信念为$mu(omega_1) = 0.75, mu(omega_2) = 0.25$)：
$ bb(E) [u(a_1) | s_1] = 0.75 times 2 + 0.25 times 0 = 1.5 $
$ bb(E) [u(a_2) | s_1] = 0.75 times 0 + 0.25 times 3 = 0.75 $
因为$1.5 > 0.75$，买家会选择行动$a_1$。此时的期望效用为1.5。 

当观察到$s_2$时 (后验信念为$mu(omega_1)= 7/16, mu(omega_2) = 9/16$)： 
$ bb(E) [u(a_1) | s_2] = 7/16 times 2 + 9/16 times 0 = (14)/(16) $
$ bb(E) [u(a_2) | s_2] = 7/(16) times 0 + 9/(16) times 3 = (27)/(16) $
因为$(27)/(16)> (14)/(16)$，买家会选择行动$a_2$。此时的期望效用为$(27)/(16)$。 

计算总的期望效用（事前期望）:买家在获得信号之前的总期望效用，是所有可能信号带来的期望效用的加权平均，权重为各信号出现的概率。
$ bb(E) [U("有信息")] = P(s_1) dot bb(E) [U | s_1] + P(s_2) dot bb(E) [U | s_2] $
$ bb(E) [U("有信息")] = 0.84 times 1.5 + 0.16 times (27/(16)) = 1.53 $

因此，有信息时买家的最大期望效用为$bb(E) [U("有信息")] = 1.53$。 

现在我们可以计算这个信号机制对买家的确切价值。

$ V("信息") = bb(E) [U("有信息")] - bb(E) [U("有信息")] $
$ V("信息") = 1.53 - 1.40 = 0.13 $
