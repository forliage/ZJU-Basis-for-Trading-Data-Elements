...
*Solution*: 定义博弈要素
- Players:$I={1,2}$。参与人1是行参与人，参与人2是列参与人。
- Types: 参与人1的类型空间是$T_1 = {A,B}$；参与人2的类型空间是$T_2 = {C}$(参与人2只有一个类型，因此其类型是确定的)
- Actions: 参与人1的行动空间是$A_1 = {T,B}$(上，下)；参与人2的行动空间是$A_2={L,R}$(左，右)
- Beliefs:自然选择的类型组合的先验概率为$p(A,C)=1/3$和$p(B,C)=2/3$。由于参与人2只有一个类型$C$，参与人2对于参与人1类型的信念为：$ p(t_1 = A | t_2 = C) = p(A, C) = 1/3 $  $ p(t_1 = B | t_2 = C) = p(B,C) = 2/3 $

参与人1的策略 是一个函数$s_1: T_1 -> A_1$，它为其每种可能的类型指定一个行动。一个纯策略可以表示为$(s_1 (A), s_1 (B))$。例如，$(T, B)$ 表示$A$类型选择$T$，$B$类型选择$B$。参与人2的策略 由于只有一个类型，其策略就是从其行动空间中选择一个行动$s_2 in A_2$。 

我们通过考察参与人2的纯策略来寻找均衡。

情况 1: 假设参与人2选择行动$L$
+ 参与人1的最佳反应:
   - 如果参与人1是$A$类型，他将比较选择$T$(收益2) 和选择$B$(收益0) 的收益。他的最佳反应是$T(2>0)$。
   - 如果参与人1是$B$类型，他将比较选择$T$(收益0) 和选择$B$(收益2) 的收益。他的最佳反应是$B(2>0)$。 
   - 因此，如果参与人2选择$L$，参与人1的最佳策略是 $(T, B)$。
+ 验证参与人2的策略:
   - 现在我们需要检验，当参与人1采取策略$(T, B)$时，$L$是否是参与人2的最佳反应。参与人2需要计算选择$L$和$R$的期望收益。
   - 选择$L$的期望收益 $bb(E)[U_2 (L)]$: $bb(E) [U_2 (L)] = p(A) dot U_2 (T,L|A)+ p(B) dot U_2 (B,L|B)= 1/3 dot 0 + 2/3 dot 0 = 0$
   - 选择$R$的期望收益 $bb(E)[U_2 (L)]$: $bb(E) [U_2 (R)] = p(A) dot U_2 (T,R|A)+p(B) dot U_2 (B,R|B) = 1/3 dot 3 + 2/3 dot 1 = 5/3$
   - 由于$bb(E)[U_2 (R)] = 5/3 > bb(E) [U_2 (L)] = 0$，参与人2的最佳反应是$R$，而不是$L$。
   - 结论: (策略组合: $(T,B), L$) 不是一个贝叶斯纳什均衡。

情况 2: 假设参与人2选择行动$R$
+ 参与人1的最佳反应:
   - 如果参与人1是$A$类型，他将比较选择$T$(收益0)和选择$B$(收益1) 的收益。他的最佳反应是$B(1>0)$。 
   - 如果参与人1是$B$类型，他将比较选择$T$(收益3)和选择$B$(收益0) 的收益。他的最佳反应是$T(3>0)$。 
   - 因此，如果参与人2选择$R$，参与人1的最佳策略是$(B, T)$。
+ 验证参与人2的策略:
   - 当参与人1采取策略$(B, T)$时，检验$R$是否是参与人2的最佳反应。
   - 选择$L$的期望收益$bb(E) [U_2 (L)]$: $bb(E) [U_2 (L)] = p(A) dot U_2 (B,L|A) + p(B) dot U_2 (T,L|B) = 1/3 dot 4 + 2/3 dot 3 = (10)/3 $
   - 选择$R$的期望收益$bb(E) [U_2 (R)]$: $bb(E) [U_2 (R)] = p(A) dot U_2 (B,R|A) + p(B) dot U_2 (T, R|B) = 1/3 dot 0 + 2/3 dot 1 = 2/3$
   - 由于$bb(E) [U_2 (L)] = (10)/3 > bb(E) [U_2 (R)] = 2/3$，参与人2的最佳反应是$L$，而不是$R$。
   - 结论: (策略组合: $(B,T), R$) 不是一个贝叶斯纳什均衡。

综合以上两种情况，该博弈没有纯策略贝叶斯纳什均衡。

由于没有纯策略均衡，我们寻找混合策略均衡。
- 设参与人1 $A$类型的策略是：以概率$p_A$选择$T$，以概率$1 - p_A$选择$B$
- 设参与人2 $B$类型的策略是：以概率$p_B$选择$T$，以概率$1 - p_B$选择$B$
- 设参与人2的策略是：以概率$q$选择$L$，以概率$1-q$选择$R$

在混合策略均衡中，进行混合的参与人必须对其选择的行动感到无差异。

步骤 1: 确定参与人1的无差异条件
- A类型参与人1无差异:$ bb(E)[U_1 (T|A)] = bb(E) [U_1 (B|A)] => q = 1/3 $
   - 如果$q > 1/3$，$A$类型严格偏好$T$ ($p_A = 1$)
   - 如果$q < 1/3$，$A$类型严格偏好$B$ ($p_A=0$)
- B类型参与人1无差异:$ bb(E) [U_1 (T|B)] = bb(E) [U_1 (B|B)] => q = 3/5 $
   - 如果$q < 3/5$，$B$类型严格偏好$T$ ($p_B=1$)
   - 如果$q > 3/5$，$B$类型严格偏好$B$ ($p_B=0$)

步骤 2: 确定参与人2的无差异条件

参与人2选择混合策略，当且仅当他对选择L和R的期望收益无差异。

$ bb(E) [U_2 (L)] = bb(E) [U_2 (R)] $

$ bb(E) [U_2 (L)] = p(A) dot [p_A U_2 (T,L|A) + (1 - p_A) U_2 (B,L|A)] + p(B) dot [p_B U_2 (T, L|B) + (1 - p_B) U_2 (B,L|B)] $

$ bb(E) [U_2 (L)] = 1/3 [p_A dot 0 + (1 - p_A) dot 4] + 2/3 [p_B dot 3 + (1 - p_B) dot 0] = 4/3 (1 - p_A) + 2 p_B $

$ bb(E)[U_2 (R)] = p(A) dot [p_A U_2 (T,R|A) + (1 - p_A) U_2 (B,R|A)] + p(B) dot [p_B U_2 (T,R|B) + (1 - p_B)U_2 (B,R|B)] $

$ bb(E)[U_2 (R)] = 1/3 [p_A dot 3 + (1 - p_A) dot 0] + 2/3 [p_B dot 1 + (1 - p_B) dot 1] = p_A + 2/3 $

令两者相等：
$ 4/3 (1 - p_A) + 2 p_B = p_A + 2/3 $
得到：
$ 2 = 7 p_A - 6 p_B $

我们需要找到一组$(p_A, p_B, q)$同时满足所有人的最佳反应条件。由于没有纯策略均衡，参与人2必须混合$(0<q<1)$，这意味着参与人2必须无差异。因此，方程$2 = 7 p_A - 6 p_B$必须成立。

同时，参与人1的策略$(p_A,p_B)$必须是对$q$的最佳反应。我们来检验使参与人1的某一类型无差异的$q$值： 

假设$q = 1/3$($A$类型无差异):此时$p_A in [0,1]$。对于B类型，由于$q = 1/3 < 3/5$，B类型的最佳反应是T，即$p_B = 1$。代入参与人2的无差异条件: $2=7 p_A - 6 (1) => 8 = 7 p_A => p_A = 8/7$。这是一个无效的概率，因此不存在$q = 1/3$的均衡。

假设$q = 3/5$ ($B$类型无差异):此时$p_B in [0,1]$。 对于A类型，由于$q = 3/5 > 1/3$，A类型的最佳反应是T，即$p_A = 1$。代入参与人2的无差异条件: $2 = 7(1) - 6 p_B => 6 p_B = 5 => p_B = 5/6$。这是一个有效的概率($0<= 5/6 <= 1$)。 

我们找到了一个混合策略贝叶斯纳什均衡:
- 参与人1的策略$s_1^ast$:
   - 如果类型是A, 选择$T$ ($p_A=1$)
   - 如果类型是B, 以 $5/6$的概率选择$T$，以 1/6的概率选择$B$ ($p_B=5/6$)
- 参与人2的策略$s_2^ast$:
   - 以 3/5的概率选择L，以 $2/5$的概率选择$R$ ($q=3/5$)
