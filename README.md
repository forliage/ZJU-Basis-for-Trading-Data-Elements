...
1. **柠檬市场**
   ...
   * **考察**：信息不对称下的逆向选择与“劣币驱逐良币”机制。
   * **解法思路**：需能用二手车市场例子说明卖方隐性信息如何导致高质量产品退出市场。

2. **子博弈完美均衡**

   * **考察**：扩展式博弈中，如何剔除不可置信威胁。
   * **解法思路**：解释在每个子博弈上都要是纳什均衡，并与普通纳什均衡区别开来。

3. **组合无套利**

   * **考察**：数据定价中防止买家“拆分–合并”套利的原则。
   * **解法思路**：说明若一个查询可拆成子查询，其价格总和不得低于整体查询价格。

4. **DSIC**

   * **考察**：机制设计中占优策略激励相容的定义与意义。
   * **解法思路**：强调“无论他人策略如何，诚实报告即最佳策略”的约束条件。

5. **比较优势**

   * **考察**：国际贸易或生产分工背后的机会成本逻辑。
   * **解法思路**：用机会成本较低者在该领域生产效率更高来阐述。

---

### 二、单项选择（9 × 2分）

1. **差分隐私**：测试对“通过添加随机噪声实现隐私保护”原理的掌握。
2. **数字竞猜博弈的纯策略纳什均衡**：考察对“最接近平均数一半”博弈中 0 是唯一均衡的分析方法。
3. **贝叶斯纯策略纳什均衡**：检验对不完全信息博弈中各类型最佳应对与信念计算的能力。
4. **合作博弈的核与沙普利值**：区分联盟理性约束（Core）与边际贡献分配（Shapley）的计算。
5. **多臂老虎机算法**：理解 UCB 如何平衡“选择表现好”与“选择尝试少”两者。
6. **单物品拍卖效率**：验证第一价格拍卖的对称递增均衡为何能保证事后有效率。
7. **VCG 机制缺陷**：考察 VCG 在参拍者增多时可能违反收入单调性的特点。
8. **模型版本化定价无套利**：测试对通过多个低质量模型平均构造高质量模型的套利分析。
9. **福利三角**：在三级价格歧视框架下，如何写出“消费者效用 ≥ 0”、“卖家利润 ≥ 最优垄断利润”、“总福利 ≤ 最大可能福利”三条线性约束。

---

### 三、简答与计算：纳什均衡（14分）

* **考察**：将公共物品（只要一人购买全体共享）场景建模为策略式博弈；

  1. 识别“非排他性”属性及其导致的“搭便车”问题；
  2. 枚举纯策略均衡（恰有一人购买）；
  3. 利用无差异原则推导对称混合策略均衡概率 $q=1-(p/u)^{1/(n-1)}$。

---

### 四、简答与计算：不确定性下的数据定价（15分）

1. **在线定价→多臂老虎机（MWU）**

   * **考察**：将离散价格选择问题视作对抗性 MAB，定义代价 $c_t(i)\in\{0,1\}$，并用乘性权重更新权重。
2. **聚集拍卖→最优拍卖**

   * **考察**：Myerson 最优拍卖设计：对 $U[200,500]$ 分布计算虚拟估值 $c(t)=2t-500$，解出保留价 250 及次价密封拍卖规则。
3. **现实机制→版本化定价**

   * **考察**：无需先验，利用多版本自我选择实现价格歧视，解决卖家对单个买家估值不确定的问题。
...
1. **信息设计 vs. 机制设计**
   ...
   * **考察**：二者同为影响参与人决策的工具，但前者操纵“信念”，后者操纵“激励”；
   * **显示原理差异**：信息设计的“直接”是推荐接收者行动，机制设计的“直接”是参与者报告类型。
2. **推荐信问题**

   * **考察**：

     * (a) 推导先验下企业默认是否雇用的阈值 $p\ge2/3$；
     * (b) 用凹包络方法构造两个后验 $(0,2/3)$ 及信号概率 $(1,\tfrac12)$ 的最优信号机制。

---

### 六、课程反馈（30分）

收集学生对课程内容、安排、考核及课堂氛围的主观评价，不作知识性考察。

## VII. 写在最后 (Conclusion)

这门课程的笔记、作业、实验和考试内容的更新至此已经结束了。作为短学期的课程来说这门课的难度还是不小的，不过得分还是并不困难的。

倒数第二节课的时候助教gg会讲考试的重点，Final Exam/Review下的文档就是考前一天晚上临时找出来复习用的，对比期末题目可知我们的文档是有效果的，实际上，全部的题目的解答都可以在这个仓库的笔记中找到。我更推荐的方法是几个好朋友一起拉一个repo来共享，这样可以互相补充（避免了我丢掉了柠檬市场的解释然后回去翻笔记的问题）。

希望用到和以后可能用的这个仓库的朋友、小朋友们可以学到满满的知识，得到满意的成绩！
