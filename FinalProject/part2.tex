...
其中$t(\theta)$是卖方最终从类型为$\theta$的买方那里收取的期望费用。显然，卖方收取的费用不能超过信息的价值，即
...
假设在广告例子中，$\omega=$年轻单身时信息价值高，卖方定价$t$(年轻单身)= 1000元；$\omega=$
...
在这个特定的例子中，两种类型的买方$\theta=0$和$\theta=1$恰好具有相同的信息价值$\xi(\theta)=2.0$。
